...
cd ${GOPATH}/src/github.com/gatechain/gatemint

git fetch
git checkout rel/stable
git merge origin/rel/stable
scp -p ${GOPATH}/src/github.com/gatechain/gatemint/scripts/build_release_setup.sh ubuntu@${TARGET}:~/
...
(bash "${HOME}/go/src/github.com/gatechain/gatemint/scripts/build_release.sh" 2>&1)|tee -a "${HOME}/buildlog_${BUILDTIMESTAMP}"
(bash "${HOME}/go/src/github.com/gatechain/gatemint/scripts/build_release_sign.sh" 2>&1)|tee -a "${HOME}/buildlog_${BUILDTIMESTAMP}"
(bash "${HOME}/go/src/github.com/gatechain/gatemint/scripts/build_release_upload.sh" 2>&1)|tee -a "${HOME}/buildlog_${BUILDTIMESTAMP}"
if [ -f "${HOME}/rstamp" ]; then
    . "${HOME}/rstamp"
fi
if [ -z "${RSTAMP}" ]; then
    RSTAMP=$(${HOME}/go/src/github.com/gatechain/gatemint/scripts/reverse_hex_timestamp)
fi
if [ -z "${RSTAMP}" ]; then
