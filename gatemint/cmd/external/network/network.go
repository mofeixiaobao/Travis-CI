...
// Cfg contains the persisted configuration of the deployed network
type Cfg struct {
	Name string
...
func loadNetworkCfg(configFile string) (Cfg, error) {
	cfg := Cfg{}
...
func saveNetworkCfg(cfg Cfg, configFile string) error {
...
func (n Network) Start(binDir string, redirectOutput bool) error {
	...
	for _, relayDir := range n.cfg.RelayDirs {
		nc := control.MakeNodeController(binDir, n.getNodeFullPath(relayDir))
		args := control.AlgodStartArgs{
...
func (n Network) getRelayAddress(nc control.NodeController) (relayAddress string, err error) {
	for i := 1; ; i++ {
...
func (n Network) startNodes(binDir, relayAddress string, redirectOutput bool) error {
	...
	for _, nodeDir := range n.nodeDirs {
		nc := control.MakeNodeController(binDir, n.getNodeFullPath(nodeDir))
		_, err := nc.StartAlgod(args)
		if err != nil {
			return err
		}
...
func (n Network) StartNode(binDir, nodeDir string, redirectOutput bool) (err error) {
	controller := control.MakeNodeController(binDir, nodeDir)
	peers := n.GetPeerAddresses(binDir)
	peerAddresses := strings.Join(peers, ";")

	_, err = controller.StartAlgod(control.AlgodStartArgs{
...
func (n Network) Stop(binDir string) {
	...
	stopNodeContoller := func(nc control.NodeController) {
		defer func() {
			c <- struct{}{}
		}()
		nc.FullStop()
...
func (n Network) GetNodeController(binDir, nodeName string) (nc control.NodeController, err error) {
	nodeDir, err := n.GetNodeDir(nodeName)
	if err != nil {
		return
	}
	nc = control.MakeNodeController(binDir, nodeDir)
	return
}
