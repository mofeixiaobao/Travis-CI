...
var dataDirectory = flag.String("d", "", "Root Gatemint daemon data path")
...
func main() {
	...
	log := logging.Base()
	// before doing anything further, attempt to acquire the gmd lock
	// to ensure this is the only node running against this data directory
	lockPath := filepath.Join(absolutePath, "gmd.lock")
	fileLock := flock.New(lockPath)
	locked, err := fileLock.TryLock()
	if err != nil {
		fmt.Fprintf(os.Stderr, "unexpected failure in establishing gmd.lock: %s \n", err.Error())
		os.Exit(1)
	}
	if !locked {
		fmt.Fprintln(os.Stderr, "failed to lock gmd.lock; is an instance of gmd already running in this data directory?")
		os.Exit(1)
	}
	...
	apiToken, wroteNewToken, err := tokens.ValidateOrGenerateAPIToken(s.RootPath, tokens.GmdTokenFilename)
	...
	err = s.Initialize(cfg, appinterface.ProxyBaseApplication{})
	if err != nil {
...
func initTelemetry() {
	...
	configPath, _ := config.GetConfigFilePath(logging.TelemetryConfigFilename)
	dataDirectory := config.GetCurrentVersion().DataDirectory
	mc := metrics.MakeCounter(metrics.MetricName{Name: logging.MetricName, Description: logging.MetricDescription})
	logging.InitTelemetry(mc, channel, configPath, dataDirectory)
}
