...
function check_install_valid() {
    ...
    if [ ! -f "${BINDIR}/gmd" ]; then
        echo "Missing ${BINDIR}/gmd"
        return 1
    fi
    return 0
}
...
function determine_current_version() {
    CURRENTVER="$(( ${BINDIR}/gmd -v 2>/dev/null || echo 0 ) | head -n 1)"
...
function shutdown_node() {
    echo Stopping node...
    if [ "$(pgrep -x gmd)" != "" ] || [ "$(pgrep -x kmd)" != "" ] ; then
        if [ -f ${BINDIR}/goal ]; then
            for DD in ${DATADIRS[@]}; do
                if [ -f ${DD}/gmd.pid ] || [ -f ${DD}/**/kmd.pid ] ; then
                    ...
                    # Clean up zombie (gmd|kmd).net files
                    rm -f ${DD}/gmd.net ${DD}/**/kmd.net
                fi
            done
        fi
    else
...
function backup_binaries() {
    ...
    BACKUPFILES="gmd kmd carpenter doberman goal update.sh updater diagcfg"
...
function check_for_new_ledger() {
    ...
    EXISTING_VER=$(${UPDATESRCDIR}/bin/gmd -d ${CURDATADIR} -g ${CURDATADIR}/genesis.json -G)
    ...
    NEW_VER=$(${UPDATESRCDIR}/bin/gmd -d ${CURDATADIR} -g ${UPDATESRCDIR}/genesis/${GENESIS_NETWORK_DIR}/genesis.json -G)
    if [ $? -ne 0 ]; then
