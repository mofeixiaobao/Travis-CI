...
func validateGenesis(genesis bookkeeping.Genesis) {
	...
	for _, alloc := range genesis.Allocation {
		...
		if alloc.State.Power.Raw < config.Consensus[genesis.Proto].MinBalance {
			log.Fatalf("account %s has less than MinBalance: %d < %d", alloc.Address, alloc.State.Power.Raw, config.Consensus[genesis.Proto].MinBalance)
		}
	}
	...
	for {
		...
		alloc := bookkeeping.GenesisAllocation{
			...
			State: basics.AccountData{
				...
				Power:           basics.Power{Raw: record.Algos * 1e6},
...
func parseMetadata(metadataFile string) (genesis bookkeeping.Genesis, err error) {
	...
	genesis.ChainID = metadata.SchemaID
...
type record struct {
	...
	VoteID          basics.OneTimeSignatureVerifier
