...
func MakeGenesisBalances(balances map[basics.Address]basics.AccountData) GenesisBalances {
	return MakeTimestampedGenesisBalances(balances, time.Now().Unix())
}

// MakeTimestampedGenesisBalances returns the information needed to bootstrap the ledger based on a given time
func MakeTimestampedGenesisBalances(balances map[basics.Address]basics.AccountData, timestamp int64) GenesisBalances {
	return GenesisBalances{balances: balances, timestamp: timestamp}
}
