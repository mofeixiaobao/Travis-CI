...
func (root Root) Address() basics.Address {
	//if len(root.secrets.SignatureVerifier) != len(basics.Address{}) {
	//	panic("length not equal")
	//}
	return basics.ConverAddress(root.secrets.SignatureVerifier[:])
...
func RestoreParticipation(store db.Accessor) (acc Participation, err error) {
	...
	err = store.Atomic(func(tx *sql.Tx) error {
		...
		row = tx.QueryRow("select parent, vrf, voting, keyDilution from ParticipationAccount")
		err = row.Scan(&rawParent, &rawVRF, &rawVoting, &acc.KeyDilution)
		if err != nil {
			return fmt.Errorf("RestoreParticipation: could not read account raw data: %v", err)
		}

		copy(acc.Parent[:48], rawParent)
		return nil
	})
	if err != nil {
