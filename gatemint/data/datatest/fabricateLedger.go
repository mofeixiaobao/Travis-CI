...
import (
	...
	"github.com/gatechain/gatemint/agreement/agreementtest"
...
func FabricateLedger(log logging.Logger, ledgerName string, accounts []account.Participation, genesis data.GenesisBalances, lastRound basics.Round) (*data.Ledger, error) {
	const inMem = true
	const archival = true
	ledger, err := data.LoadLedger(log, ledgerName, inMem, protocol.ConsensusCurrentVersion, genesis, "", crypto.Digest{}, nil, archival, nil, "", bookkeeping.Genesis{})
	if err != nil {
		return nil, err
	}
