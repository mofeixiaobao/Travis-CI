...
type proxyTxBacklogMsg struct {
	rawmsg                 *network.IncomingMessage      // the raw message from the network
	unverifiedProxyTxGroup []transactions.Tx             // the unverified ( and signed ) proxy transaction group
	proto                  config.ConsensusParams        // the consensus parameters that corresponds to the latest round. Filled in during checkAlreadyCommitted execution.
	spec                   transactions.SpecialAddresses // corresponds to the latest round
	verificationErr        error                         // The verification error generated by the verification function, if any.
}
...
type TxHandler struct {
	...
	// add proxy tx queue
	proxyBacklogQueue          chan *proxyTxBacklogMsg
	proxyPostVerificationQueue chan *proxyTxBacklogMsg
	proxyBacklogWg             sync.WaitGroup
...
func MakeTxHandler(txPool *pools.TransactionPool, ledger *Ledger, net network.GossipNode, genesisID string, genesisHash crypto.Digest, executionPool execpool.BacklogPool) *TxHandler {
	...
	handler := &TxHandler{
		...
		proxyBacklogQueue:          make(chan *proxyTxBacklogMsg, txBacklogSize),
		proxyPostVerificationQueue: make(chan *proxyTxBacklogMsg, txBacklogSize),
	...
	net.RegisterHandlers([]network.TaggedMessageHandler{
		...
		network.TaggedMessageHandler{Tag: protocol.ProxyTxnTag, MessageHandler: network.HandlerFunc(handler.processIncomingProxyTxn)},
	})
...
func (handler *TxHandler) backlogWorker() {
	...
	defer handler.proxyBacklogWg.Done()
	for {
		...
		// todo add proxyPostVerificationQueue
		select {
		case wi, ok := <-handler.proxyPostVerificationQueue:
			if !ok {
				return
			}
			handler.postprocessCheckedProxyTxn(wi)
			// restart the loop so that we could empty out the post verification queue.
			continue
		default:
		}
		...
		// we have no more post verification items. wait for either backlog queue item or post verification item.
		select {
		case wi, ok := <-handler.proxyBacklogQueue:
			if !ok {
				return
			}
			if handler.checkProxyTxAlreadyCommitted(wi) {
				continue
			}
			// enqueue the task to the verification pool.
			handler.txVerificationPool.EnqueueBacklog(handler.ctx, handler.asyncVerifyProxySignature, wi, nil)
...
func (handler *TxHandler) postprocessCheckedProxyTxn(wi *proxyTxBacklogMsg) {
	if wi.verificationErr != nil {
		// disconnect from peer.
		logging.Base().Warnf("Received a malformed tx group %v: %v", wi.unverifiedProxyTxGroup, wi.verificationErr)
		handler.net.Disconnect(wi.rawmsg.Sender)
		return
	}
	...
	// at this point, we've verified the transaction, so we can safely treat the transaction as a verified transaction.
	verifiedTxGroup := wi.unverifiedProxyTxGroup

	// save the transaction, if it has high enough fee and not already in the cache
	// here is proxy txGroup is certificate has only one tx
	err := handler.txPool.RememberSingle(verifiedTxGroup[0])
	if err != nil {
		logging.Base().Debugf("could not remember tx: %v", err)
		return
	}
	...
	// We reencode here instead of using rawmsg.Data to avoid broadcasting non-canonical encodings
	handler.net.Relay(handler.ctx, protocol.ProxyTxnTag, verifiedTxGroup[0], false, wi.rawmsg.Sender)
}
...
// asyncVerifySignature verifies that the given transaction group is valid, and update the txBacklogMsg data structure accordingly.
func (handler *TxHandler) asyncVerifyProxySignature(arg interface{}) interface{} {
	tx := arg.(*proxyTxBacklogMsg)
	//for _, txn := range tx.unverifiedProxyTxGroup {
	//	tx.verificationErr = verify.Txn(&txn, tx.spec, tx.proto)
	//	if tx.verificationErr != nil {
	//		break
	//	}
	//}
	select {
	case handler.proxyPostVerificationQueue <- tx:
	default:
		// we failed to write to the output queue, since the queue was full.
		// adding the metric here allows us to monitor how frequently it happens.
		transactionMessagesDroppedFromPool.Inc(nil)
...
func (handler *TxHandler) processIncomingTxn(rawmsg network.IncomingMessage) network.OutgoingMessage {
	...
	for {
		...
		if err != nil {
			logging.Base().Error("Received a non-decodable txn: %v", err)
...
func (handler *TxHandler) processIncomingProxyTxn(rawmsg network.IncomingMessage) network.OutgoingMessage {
	//ntx := 0
	unverifiedProxyTxGroup := make([]transactions.Tx, 1)
	//for {
	//	if len(unverifiedProxyTxGroup) == ntx {
	//		n := make([]transactions.Tx, len(unverifiedProxyTxGroup)*2)
	//		copy(n, unverifiedProxyTxGroup)
	//		unverifiedProxyTxGroup = n
	//	}
	//	//err := dec.Decode(&unverifiedProxyTxGroup[ntx])
	//	//if err == io.EOF {
	//	//	break
	//	//}
	//	//if err != nil {
	...
	//}
	//if ntx == 0 {
	//	logging.Base().Warnf("Received empty proxy tx group")
	//	return network.OutgoingMessage{Action: network.Disconnect}
	//}
	//unverifiedProxyTxGroup = unverifiedProxyTxGroup[:ntx]

	unverifiedProxyTxGroup[0] = rawmsg.Data

	select {
	case handler.proxyBacklogQueue <- &proxyTxBacklogMsg{
		rawmsg:                 &rawmsg,
		unverifiedProxyTxGroup: unverifiedProxyTxGroup,
	}:
	default:
		// if we failed here we want to increase the corresponding metric. It might suggest that we
		// want to increase the queue size.
...
// checkProxyTxAlreadyCommitted test to see if the given proxy transaction ( in the txBacklogMsg ) was already commited, and
// whether it would qualify as a candidate for the transaction pool.
func (handler *TxHandler) checkProxyTxAlreadyCommitted(tx *proxyTxBacklogMsg) (processingDone bool) {
	txids := make([]transactions.Txid, len(tx.unverifiedProxyTxGroup))
	for i := range tx.unverifiedProxyTxGroup {
		//tx.unverifiedProxyTxGroup[i].Txn.InitCaches()
		txids[i] = tx.unverifiedProxyTxGroup[i].ComputeID()
	}
	logging.Base().Debugf("got a tx group with IDs %v", txids)

	// do a quick test to check that this transaction could potentially be committed, to reject dup pending transactions
	err := handler.txPool.TestProxyTx(tx.unverifiedProxyTxGroup)
	if err != nil {
		logging.Base().Debugf("txPool rejected transaction: %v", err)
		return true
	}
...
func (handler *TxHandler) processProxyDecoded(unverifiedProxyTxGroup []transactions.Tx) (outmsg network.OutgoingMessage, processingDone bool) {
	tx := &proxyTxBacklogMsg{
		unverifiedProxyTxGroup: unverifiedProxyTxGroup,
	}
	if handler.checkProxyTxAlreadyCommitted(tx) {
		return network.OutgoingMessage{}, true
	}
	...
	// save the transaction, if it has high enough fee and not already in the cache
	for _, singleProxyTx := range unverifiedProxyTxGroup {
		err := handler.txPool.RememberSingle(singleProxyTx)
		if err != nil {
			logging.Base().Debugf("could not remember proxy tx: %v", err)
			return network.OutgoingMessage{}, true
		}
	}
	return network.OutgoingMessage{}, false
...
type SolicitedTxHandler interface {
	...
	HandleProxy(txgroup []transactions.Tx) error
}
...
func (handler *solicitedTxHandler) HandleProxy(txgroup []transactions.Tx) error {
	outmsg, _ := handler.txHandler.processProxyDecoded(txgroup)
	if outmsg.Action == network.Disconnect {
		return fmt.Errorf("invlid proxy transaction")
	}
	return nil
}
