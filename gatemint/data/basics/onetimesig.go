...
type OneTimeSignatureSubkeyBatchID struct {
	SubKeyPK crypto.Ed25519PublicKey `codec:"pk"`
...
func (batch OneTimeSignatureSubkeyBatchID) ToBeHashed() (crypto.HashID, []byte) {
...
func (off OneTimeSignatureSubkeyOffsetID) ToBeHashed() (crypto.HashID, []byte) {
...
type OneTimeSignatureVerifier crypto.Ed25519PublicKey
...
type OneTimeSignatureSecretsPersistent struct {
	...
	EphemeralSk crypto.Ed25519PrivateKey `codec:"topsk"`
	...
	OffsetsPK2    crypto.Ed25519PublicKey `codec:"offpk2"`
	OffsetsPK2Sig crypto.Ed25519Signature `codec:"offpk2sig"`
}
...
type ephemeralSubkey struct {
	...
	PKSigOld crypto.Ed25519Signature `codec:"PKSig"`
	...
	PKSigNew crypto.Ed25519Signature `codec:"sig2"`
}
...
func GenerateOneTimeSignatureSecretsRNG(startBatch uint64, numBatches uint64, rng crypto.RNG) *OneTimeSignatureSecrets {
	s := new(OneTimeSignatureSecrets)

	master, ephemeralSec := crypto.Ed25519GenerateKeyRNG(rng)

	subkeys := make([]ephemeralSubkey, numBatches)
	for i := uint64(0); i < numBatches; i++ {
		pk, sk := crypto.Ed25519GenerateKeyRNG(rng)
		batchnum := startBatch + i

		newid := OneTimeSignatureSubkeyBatchID{SubKeyPK: pk, Batch: batchnum}
		newsig := crypto.Ed25519Sign(ephemeralSec, crypto.HashRep(newid))
	...
	s.Batches = subkeys
	s.rng = rng
	s.EphemeralSk = ephemeralSec
	return s
}

func (s *OneTimeSignatureSecrets) GenerateOneTimeSignatureSecretsAgain(startBatch uint64, numBatches uint64, rng crypto.RNG) {
	subkeys := make([]ephemeralSubkey, numBatches)
	for i := uint64(0); i < numBatches; i++ {
		pk, sk := crypto.Ed25519GenerateKeyRNG(rng)
		batchnum := startBatch + i
		...
		newid := OneTimeSignatureSubkeyBatchID{SubKeyPK: pk, Batch: batchnum}
		newsig := crypto.Ed25519Sign(s.EphemeralSk, crypto.HashRep(newid))
...
func GenerateOneTimeSignatureSecrets(startBatch uint64, numBatches uint64) *OneTimeSignatureSecrets {
	return GenerateOneTimeSignatureSecretsRNG(startBatch, numBatches, crypto.SystemRNG)
}
...
func (s *OneTimeSignatureSecrets) getRNG() crypto.RNG {
	if s.rng != nil {
		return s.rng
	}
	return crypto.SystemRNG
}

// Sign produces a OneTimeSignature of some Hashable message under some
// OneTimeSignatureIdentifier.
func (s *OneTimeSignatureSecrets) Sign(id OneTimeSignatureIdentifier, message crypto.Hashable) OneTimeSignature {
	s.mu.RLock()
	defer s.mu.RUnlock()
	...
	if id.Batch+1 == s.FirstBatch && id.Offset >= s.FirstOffset && id.Offset-s.FirstOffset < uint64(len(s.Offsets)) {
		...
		sig := crypto.Ed25519Sign(s.Offsets[offidx].SK, crypto.HashRep(message))
	...
	if id.Batch >= s.FirstBatch && id.Batch-s.FirstBatch < uint64(len(s.Batches)) {
		...
		pk, sk := crypto.Ed25519GenerateKeyRNG(s.getRNG())
		sig := crypto.Ed25519Sign(sk, crypto.HashRep(message))
		...
		return OneTimeSignature{
			Sig:    sig,
			PK:     pk,
			PK1Sig: crypto.Ed25519Sign(s.Batches[batchidx].SK, crypto.HashRep(pk1id)),
...
func (v OneTimeSignatureVerifier) Verify(id OneTimeSignatureIdentifier, message crypto.Hashable, sig OneTimeSignature) bool {
	...
	if !crypto.Ed25519Verify(crypto.Ed25519PublicKey(v), crypto.HashRep(batchID), sig.PK2Sig) {
		return false
	}
	if !crypto.Ed25519Verify(batchID.SubKeyPK, crypto.HashRep(offsetID), sig.PK1Sig) {
		return false
	}
	if !crypto.Ed25519Verify(offsetID.SubKeyPK, crypto.HashRep(message), sig.Sig) {
		return false
	}
	return true
}
...
func (s *OneTimeSignatureSecrets) DeleteBeforeFineGrained(current OneTimeSignatureIdentifier, numKeysPerBatch uint64) {
	...
	if jump > uint64(len(s.Batches)) {
		...
		s.GenerateOneTimeSignatureSecretsAgain(current.Batch, config.GetDefaultLocal().RefreshPartInterval, s.getRNG())
		jump = uint64(0)
		//if s.Batches != nil {
	...
	for off := current.Offset; off < numKeysPerBatch; off++ {
		pk, sk := crypto.Ed25519GenerateKeyRNG(s.getRNG())
		pksig := crypto.Ed25519Sign(s.Batches[0].SK, crypto.HashRep(OneTimeSignatureSubkeyOffsetID{
			SubKeyPK: pk,
			Batch:    current.Batch,
			Offset:   off,
		}))
