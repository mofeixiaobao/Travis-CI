...
type Balances interface {
	...
	// Move Power from one account to another, doing all necessary overflow checking (convenience method)
	...
	Move(src, dst basics.Address, amount basics.Power, srcRewards *basics.Power, dstRewards *basics.Power) error
...
type ApplyData struct {
	...
	ClosingAmount basics.Power `codec:"ca"`

	// Rewards applied to the Sender, Receiver, and CloseRemainderTo accounts.
	SenderRewards   basics.Power `codec:"rs"`
	ReceiverRewards basics.Power `codec:"rr"`
	CloseRewards    basics.Power `codec:"rc"`
}
...
func (tg TxGroup) ToBeHashed() (crypto.HashID, []byte) {
	return protocol.TxGroup, protocol.Encode(tg)
}

// ToBeHashed implements the crypto.Hashable interface.
func (tx Transaction) ToBeHashed() (crypto.HashID, []byte) {
...
func (tx Header) TxFee() basics.Power {
	return tx.Fee
}
...
func (tx Transaction) WellFormed(spec SpecialAddresses, proto config.ConsensusParams) error {
	...
	case protocol.KeyRegistrationTx:
		...
		if tx.KeyregTxnFields.Nonparticipation {
			...
			suppliesNullKeys := tx.KeyregTxnFields.VotePK == basics.OneTimeSignatureVerifier{} || tx.KeyregTxnFields.SelectionPK == crypto.VRFVerifier{}
			if !suppliesNullKeys {
	...
	if tx.Fee.LessThan(basics.Power{Raw: proto.MinTxnFee}) {
...
func (tx Transaction) TxAmount() basics.Power {
	switch tx.Type {
	case protocol.PaymentTx:
		return tx.PaymentTxnFields.Amount

	default:
		return basics.Power{Raw: 0}
	}
}
