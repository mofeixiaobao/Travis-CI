...
// TransactionList contains a list of transactions
// swagger:model TransactionList
type ProxyTransactionList struct {
	// TransactionList is a list of transactions
	//
	// required: true
	ProxyTransactions [][]byte `json:"proxyTransactions,proxyOmitempty"`
}
...
type Block struct {
	...
	ProxyTransactions ProxyTransactionList `json:"txnps"`
	...
	CertCommitteeInfo  []string
	BlockCommitteeInfo []string
}
...
func (rca ResultConAccount) String() string {
	...
	if rca.Address != nil {
		sb.WriteString(fmt.Sprintf("  Address:	%s\n", basics.ConverAddress(rca.Address)))
	}
	if !(rca.Power < 0) {
