...
func (client RestClient) TxSearch(param string, page, pagesize int, orderBy string) (response appinterface.ResultTxSearch, err error) {
	req := new(appinterface.RequestTxSearch)
	req.Param = param
	...
	req.Page = page
	req.Limit = pagesize
	err = client.post(&response, fmt.Sprintf("/txSearch"), req)
	return
}
