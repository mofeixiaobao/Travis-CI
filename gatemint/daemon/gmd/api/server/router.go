...
import (
	...
	"github.com/gatechain/gatemint/daemon/gmd/api/server/common"
	"github.com/gatechain/gatemint/daemon/gmd/api/server/lib"
	"github.com/gatechain/gatemint/daemon/gmd/api/server/lib/middlewares"
	"github.com/gatechain/gatemint/daemon/gmd/api/server/v1/routes"
...
func NewRouter(logger logging.Logger, node *node.GatemintFullNode, apiToken string) *mux.Router {
	router := mux.NewRouter().StrictSlash(true)
	// Middleware
	router.Use(middlewares.Logger(logger))
	//TODO  need revert on branch master
