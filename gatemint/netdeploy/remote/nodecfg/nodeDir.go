...
import (
	...
	"github.com/gatechain/gatemint/netdeploy/remote"
	"github.com/gatechain/gatemint/shared/algoh"
...
func (nd *nodeDir) configureAPIToken(token string) (err error) {
	...
	ioutil.WriteFile(filepath.Join(nd.dataDir, tokens.GmdTokenFilename), []byte(token), 0600)
	err = nd.saveConfig()
	return
}
