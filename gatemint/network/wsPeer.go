...
const disconnectTooMany disconnectReason = "TooManyConnection"
...
type wsPeer struct {
	...
	pexLock         deadlock.Mutex
	lastPexRequest  int64
	lastPexResponse int64
...
func dedupSafeTag(t protocol.Tag) bool {
	...
	return t == protocol.AgreementVoteTag || t == protocol.TxnTag || t == protocol.ProxyTxnTag
}
...
func (wp *wsPeer) pexRequest(wg *sync.WaitGroup) bool {
	defer wg.Done()
	wp.net.log.Infoln("send pex request to", wp.rootURL)
	tbytes := []byte(protocol.PexRequestTag)
	return wp.writeNonBlock(tbytes, false, crypto.Digest{}, time.Now())
}
