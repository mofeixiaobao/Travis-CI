...
func (rt *RequestTracker) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	trackedRequest.otherTelemetryGUID, trackedRequest.otherInstanceName, trackedRequest.otherPublicAddr, trackedRequest.otherIsSRV = getCommonHeaders(request.Header)
