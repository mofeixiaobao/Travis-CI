...
import (
	...
	tools_network "github.com/gatechain/gatemint/tools/network"
...
// connectionActivityMonitorInterval is the interval at which we send
// pex request for more peer addresses
const pexRequestInterval = 5 * time.Minute
...
type GossipNode interface {
	...
	GetPhonebook(int) []string

	GetPeersInfo(int) []string
...
func (wn *WebsocketNetwork) GetPhonebook(max int) []string {
	addresses := wn.phonebook.GetAddresses(max)
	if len(addresses) < max {
		backup := wn.phonebook.GetBackupPhonebook()
		if backup != nil {
			addresses = append(addresses, backup.GetAddresses(max-len(addresses))...)
		}
	}
...
func (wn *WebsocketNetwork) GetPeersInfo(max int) []string {
	addresses := make([]string, 0)
	wn.peersLock.RLock()
	defer wn.peersLock.RUnlock()
	for index, peer := range wn.peers {
		if index+1 > max {
			break
		}
		addresses = append(addresses, peer.rootURL)
	}
	sort.Strings(addresses)
	return addresses
}

func (wn *WebsocketNetwork) RandomPickPeersConnectedIn(request *wsPeer, max int) []string {
	addresses := make([]string, 0)
	if max < 1 {
		return addresses
	}
	...
	wn.peersLock.RLock()
	defer wn.peersLock.RUnlock()
	idxs := rand.Perm(len(wn.peers))
	for _, idx := range idxs {
		peer := wn.peers[idx]
		if peer != request && !peer.outgoing && !peer.isSRV && peer.publicAddress != "" {
			addresses = append(addresses, peer.publicAddress)
			i++
			if i >= max {
				break
			}
...
func (wn *WebsocketNetwork) setup() {
	...
	wn.pexAddresses = make(chan []string, wn.config.GossipFanout*2)
...
func (wn *WebsocketNetwork) Start() {
	...
	wn.RegisterHandlers(pexHandlers)
	if wn.listener != nil {
		wn.wg.Add(1)
		go wn.httpdThread()
	}
...
func (wn *WebsocketNetwork) setHeaders(header http.Header) {
	...
	header.Set(IsSRVHeader, strconv.FormatBool(wn.config.IsSRV))
}
...
func getCommonHeaders(headers http.Header) (otherTelemetryGUID, otherInstanceName, otherPublicAddr string, otherIsSRV bool) {
	...
	otherIsSRV, _ = strconv.ParseBool(logging.SanitizeTelemetryString(headers.Get(IsSRVHeader), 1))
	return
}
...
func (wn *WebsocketNetwork) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	otherPublicAddr := trackedRequest.otherPublicAddr
	...
	peer := &wsPeer{
		wsPeerCore: wsPeerCore{
			...
			publicAddress: getRemotePublicAddress(otherPublicAddr, trackedRequest.remoteHost),
			isSRV:         trackedRequest.otherIsSRV,
		},
		conn:              conn,
		outgoing:          false,
...
func getRemotePublicAddress(otherPublicAddr string, remoteHost string) string {
	if len(otherPublicAddr) > 8 { // https://
		ourl, err := url.Parse(otherPublicAddr)
		if err == nil {
			host := ourl.Hostname()
			portStr := ourl.Port()
			return validAddress(host, portStr, remoteHost)
			...
			//		return net.JoinHostPort(remoteHost, portStr)
			//	} else {
...
func validAddress(host, portStr, alternateHost string) string {
	_, err := strconv.ParseUint(portStr, 10, 16)
	if err == nil {
		ip := net.ParseIP(host)
		if ip == nil {
		...
		if ip == nil || ip.IsUnspecified() {
			if len(alternateHost) > 0 {
				return validAddress(alternateHost, portStr, "")
			}
		} else {
			return net.JoinHostPort(ip.String(), portStr)
		}
...
func (wn *WebsocketNetwork) checkConnectionThread() {
	defer wn.wg.Done()
	inactivityCheckTicker := time.NewTicker(connectionActivityMonitorInterval)
	defer inactivityCheckTicker.Stop()
...
func (wn *WebsocketNetwork) checkPeersConnectivity() {
	wn.peersLock.Lock()
	defer wn.peersLock.Unlock()
	wn.log.Infoln("checkPeersConnectivity")
	currentTime := time.Now()
	outgoing := 0
	remain := make([]*wsPeer, 0)
	for _, peer := range wn.peers {
		...
		if timeSinceLastPacket > maxPeerInactivityDuration {
			...
			if peer.outgoing {
				outgoing++
				if !peer.isSRV {
					remain = append(remain, peer)
				}
			}
		}
	}
	dc := outgoing - wn.config.GossipFanout
	if dc > 0 {
		for _, peer := range remain {
			wn.wg.Add(1)
			go wn.disconnectThread(peer, disconnectTooMany)
			networkIdlePeerDrops.Inc(nil)
			dc--
			if dc <= 0 {
				break
...
func (wn *WebsocketNetwork) checkSlowWritingPeers() {
	wn.peersLock.Lock()
	defer wn.peersLock.Unlock()
	wn.log.Infoln("checkSlowWritingPeers")
	currentTime := time.Now()
	for _, peer := range wn.peers {
...
func (wn *WebsocketNetwork) numOutgoingSRVPeersNoLock() int {
	count := 0
	for _, peer := range wn.peers {
		if peer.outgoing && peer.isSRV {
			count++
		}
	}
	return count
...
func (wn *WebsocketNetwork) numOutgoingSRVPeers() int {
	wn.peersLock.RLock()
	defer wn.peersLock.RUnlock()
	return wn.numOutgoingSRVPeersNoLock()
}
...
func (wn *WebsocketNetwork) pexThread() {
	defer wn.wg.Done()
	pexRequestTicker := time.NewTicker(pexRequestInterval)
	defer pexRequestTicker.Stop()
	for {
		select {
		case addresses := <-wn.pexAddresses:
			valid := make([]string, 0)
			for _, address := range addresses {
				host, portStr, err := net.SplitHostPort(address)
				if err == nil {
					va := validAddress(host, portStr, "")
					if va != "" {
			...
			if len(valid) > 0 {
				backup := wn.phonebook.GetBackupPhonebook()
				var ok bool
				var tsPhonebook *ThreadsafePhonebook
				if backup != nil {
					tsPhonebook, ok = backup.(*ThreadsafePhonebook)
				}
				if backup == nil || !ok {
					tsPhonebook = MakeThreadsafePhonebook()
					wn.phonebook.AddOrUpdateBackupPhonebook(tsPhonebook)
				}
				tsPhonebook.AddToPhonebook(valid)
				tsPhonebook.RemoveOldest(maxBackupPhonebookSize)
			}
		case <-pexRequestTicker.C:
			wn.pexRequests()
		case <-wn.ctx.Done():
			return
		}
...
func (wn *WebsocketNetwork) meshThread() {
	...
	for {
		...
		defaultPhonebook := wn.phonebook.GetPhonebook("default")
		var defaultAddresses []string
		if defaultPhonebook != nil {
			defaultAddresses = defaultPhonebook.GetAddresses(1)
		}

		// disable DNS RSV when existing --p or phonebook.json
		...
		numOutgoingSRV := wn.numOutgoingSRVPeers() + wn.numOutgoingSRVPending()
		need := desired - numOutgoingSRV
		wn.connectNeed(need, desired+numOutgoingSRV, wn.phonebook, true, &wg)
		//if need > 0 {
		//	// get more than we need so that we can ignore duplicates
		//	newAddrs := wn.phonebook.GetAddresses(desired + numOutgoingSRV)
		//	for _, na := range newAddrs {
		//		if na == wn.config.PublicAddress {
		//			continue
		//		}
		//		gossipAddr, ok := wn.tryConnectReserveAddr(na, true)
		//		if ok {
		//			wn.wg.Add(1)
		...
		if !wn.config.IsSRV {
			...
			select {
			case <-srvComplete:
			case <-time.After(meshThreadInterval / 2):
			case <-wn.ctx.Done():
				return
			}
			numOutgoing := wn.numOutgoingPeers() + wn.numOutgoingPending()
			need = desired - numOutgoing
			wn.connectNeed(need, desired+numOutgoing, wn.phonebook.GetBackupPhonebook(), false, &wg)
		}

		//if need > 0 {
		//	newAddrs := wn.phonebook.GetPhonebook("backup").GetAddresses(desired + numOutgoing)
		//	for _, na := range newAddrs {
		//		gossipAddr, ok := wn.tryConnectReserveAddr(na, false)
		//		if ok {
		//			wn.wg.Add(1)
		//			go wn.tryConnect(na, gossipAddr)
		//			need--
...
func (wn *WebsocketNetwork) connectNeed(need, want int, pb Phonebook, isSRV bool, wg *sync.WaitGroup) {
	if pb != nil && need > 0 {
		newAddrs := pb.GetAddresses(want)
		for _, na := range newAddrs {
			if na == wn.config.PublicAddress {
				continue
			}
			gossipAddr, ok := wn.tryConnectReserveAddr(na, isSRV)
			if ok {
				wn.wg.Add(1)
				wg.Add(1)
				go wn.tryConnect(na, gossipAddr, wg)
				need--
				if need == 0 {
					break
				}
...
func (wn *WebsocketNetwork) pexRequests() {
	if !wn.config.IsSRV {
		wn.peersLock.RLock()
		defer wn.peersLock.RUnlock()
		noSRV := (wn.numOutgoingSRVPeersNoLock() + wn.numOutgoingSRVPendingNoLock()) == 0
		for _, peer := range wn.peers {
			if peer.isSRV || noSRV {
				wn.wg.Add(1)
				go peer.pexRequest(&wn.wg)
			}
		}
	}
}
...
func (wn *WebsocketNetwork) getDNSAddrs(dnsBootstrap string) []string {
	srvPhonebook, err := tools_network.ReadFromSRV("gatemint", dnsBootstrap, wn.config.FallbackDNSResolverAddress)
	if err != nil {
...
func (wn *WebsocketNetwork) tryConnectReserveAddr(addr string, isSRV bool) (gossipAddr string, ok bool) {
	...
	if isSRV {
		...
		wn.tryConnectAddrs[addr] = -1
		wn.tryConnectAddrs[gossipAddr] = -1
	}
...
func (wn *WebsocketNetwork) numOutgoingSRVPendingNoLock() int {
	i := 0
	for _, d := range wn.tryConnectAddrs {
		if d > 0 {
			i++
		}
	}
...
func (wn *WebsocketNetwork) numOutgoingSRVPending() int {
	wn.tryConnectLock.Lock()
	defer wn.tryConnectLock.Unlock()
	return wn.numOutgoingSRVPendingNoLock()
}
...
func (wn *WebsocketNetwork) tryConnect(addr, gossipAddr string, wg *sync.WaitGroup) {
	...
	otherIsSrv, _ := strconv.ParseBool(logging.SanitizeTelemetryString(response.Header.Get(IsSRVHeader), 1))
	peer := &wsPeer{wsPeerCore: wsPeerCore{net: wn, rootURL: addr, isSRV: otherIsSrv}, conn: conn, outgoing: true, incomingMsgFilter: wn.incomingMsgFilter, createTime: time.Now()}
