...
func (p *ThreadsafePhonebook) AddToPhonebook(addresses []string) {
	p.lock.Lock()
	defer p.lock.Unlock()
	entries := p.entries
	for _, address := range addresses {
		entries[address] = phonebookData{}
	}
}

func (p *ThreadsafePhonebook) RemoveOldest(remain int) {
	p.lock.Lock()
	defer p.lock.Unlock()
	entries := p.entries
	size := len(entries)
	if size > remain {
		...
		sort.Slice(addresses, func(i, j int) bool {
			return entries[addresses[i]].retryAfter.After(entries[addresses[j]].retryAfter)
		})
		remain /= 2 // avoid frequent phonebook sort and delete
		for remain < size {
			delete(entries, addresses[remain])
			remain++
		}
	}
}
...
func (mp *MultiPhonebook) GetBackupPhonebook() (p Phonebook) {
	return mp.GetPhonebook(backupPhonebook)
}

func (mp *MultiPhonebook) AddOrUpdateBackupPhonebook(p Phonebook) {
	mp.AddOrUpdatePhonebook(backupPhonebook, p)
}
