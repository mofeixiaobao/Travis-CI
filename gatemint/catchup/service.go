...
func MakeService(log logging.Logger, config config.Local, net network.GossipNode, ledger Ledger,
	wsf *rpcs.WsFetcherService, auth BlockAuthenticator) (s *Service) {
	s = &Service{}
	...
	//s.deadlineTimeout = 8000 * time.Millisecond
	return s
}
...
func (s *Service) innerFetch(fetcher rpcs.Fetcher, r basics.Round) (blk *bookkeeping.Block,
	cert *agreement.Certificate, rpcc rpcs.FetcherClient, err error) {
...
func (s *Service) pipelineCallback(fetcher rpcs.Fetcher, r basics.Round, thisFetchComplete chan bool,
	prevFetchCompleteChan chan bool, lookbackChan chan bool) func() basics.Round {
...
func (s *Service) pipelinedFetch(seedLookback uint64) {
	...
	for ; nextRound < from+basics.Round(parallelRequests); nextRound++ {
		...
		taskCh <- s.pipelineCallback(fetcher, nextRound, currentRoundComplete, recentReqs[len(recentReqs)-1],
			recentReqs[len(recentReqs)-int(seedLookback)])
...
// periodicSync periodically asks the network for its latest round and syncs if we've fallen behind
// (also if our ledger stops advancing)
...
func (s *Service) sync() {
	...
	s.log.Infof("Catchup Service: finished catching up, now at round %v (previously %v). Total time catching up %v.",
		s.ledger.LastRound(), pr, elapsedTime)
}

func (s *Service) SyncComplete() chan bool {
	return s.syncComplete
}
