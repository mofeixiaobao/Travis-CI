...
func (t *timeoutTicker) stopTimer() {
	...
	if !t.timer.Stop() {
		...
		case <-t.timer.C:
		default:
			logging.Base().Info("Timer already stopped")
		}
	}
}
...
func (t *timeoutTicker) timeoutRoutine() {
	logging.Base().Debug("Starting timeout routine")
	var ti timeoutInfo
	for {
		select {
		case newti := <-t.tickChan:
			logging.Base().Debug("Received tick", "old_ti", ti, "new_ti", newti)
			...
			logging.Base().Debug("Scheduled timeout", "dur", ti.Duration, "height", ti.Height, "round", ti.Round, "step", ti.Step)
		case <-t.timer.C:
			logging.Base().Info("Timed out", "dur", ti.Duration, "height", ti.Height, "round", ti.Round, "step", ti.Step)
