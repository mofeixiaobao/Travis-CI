...
type BlockValidator interface {
	...
	Validate(context.Context, uint64, bookkeeping.Block) (ValidatedBlock, error)

	TxsAvailabled() <-chan struct{}
}
...
type BlockFactory interface {
	...
	// lose liveness.
	AssembleBlock(basics.Round, uint64, time.Time, uint64) (ValidatedBlock, error)
}
...
type LedgerReader interface {
	...
	// unavailable by the storage device. In that case, the agreement
	// protocol may lose liveness.
	Circulation(basics.Round, basics.Address) (basics.Power, basics.BalanceRecord, error)
...
// A LedgeBlockInfoPut allows put block info to db
type LedgeBlockInfoPut interface {

	// UnauthenticatedCertificateSave save UnauthenticatedCertificate to db
	// UnauthenticatedCertificate contains a cryptographic proof, but may be rewrite by other certificate
	UnauthenticatedCertificateSave(UnauthenticatedCertificate)

	// CertificateSave save Certificate to db
	// Certificate is vote info of network action on each step
