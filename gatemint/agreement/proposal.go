...
func (p unauthenticatedProposal) ToBeHashed() (crypto.HashID, []byte) {
...
// ToBeHashed implements the Hashable interface.
func (s proposerSeed) ToBeHashed() (crypto.HashID, []byte) {
...
// ToBeHashed implements the Hashable interface.
func (i seedInput) ToBeHashed() (crypto.HashID, []byte) {
...
func (p unauthenticatedProposal) validate(ctx context.Context, current round, period period, ledger LedgerReader, validator BlockValidator) (proposal, error) {
	...
	ve, err := validator.Validate(ctx, uint64(period), entry)
	if err != nil {
