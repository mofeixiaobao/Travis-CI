...
func (sel selector) ToBeHashed() (crypto.HashID, []byte) {
...
func membership(l LedgerReader, addr basics.Address, r basics.Round, p period, s step) (m committee.Membership, err error) {
	...
	total, record, err := l.Circulation(balanceRound, addr)
	if err != nil {
		err = fmt.Errorf("Service.initializeVote (r=%v): Failed to obtain total circulation or balance record for address  round %v: %v", r, balanceRound, err)
		return
	}
