...
func makeTracer(log serviceLogger, committeeLog logging.Logger, cadaverFilename string, cadaverSizeTarget uint64, verboseReportFlag bool, timingReportFlag bool) *tracer {
	t := new(tracer)
	t.log = log
	t.committeeLog = committeeLog
...
func (t *tracer) logFastTimeout(p player) {
	...
	t.log.with(logEvent).Infof("fast timeout fired on (%v, %v, %v) with value %v (napping: %v)", p.Round, p.Period, p.Step, p.FastRecoveryDeadline, p.Napping)
}
...
func (t *tracer) logProposalFrozen(prop proposalValue, propRound round, propPeriod period) {
	...
	// ConsensusTag here is ProposalFrozen, already has log
	t.log.with(logEvent).Infof("froze proposal %v for (%v, %v)", prop, propRound, propPeriod)
}

func (t *tracer) logCommitteeFrozen(prop proposalValue, propRound round, propPeriod period) {
	// ConsensusTag here is CommitteeFrozen, already has log
	t.log.Infof("froze committee %v for (%v, %v)", prop, propRound, propPeriod)
}
...
func (t *tracer) logProposalManagerResult(p player, input messageEvent, output event, pipelinedRound round, pipelinedPeriod period) {
	...
	case proposalAccepted:
		...
		consensusInfo := makeConsensusWithProposalAcceptedEvent(p, pev)
		consensusInfoString, _ := consensusInfo.JsonSerial()
		t.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoString)
	...
	case payloadAccepted, proposalCommittable:
		...
		if output.t() == payloadAccepted {
			...
			consensusInfo := makeConsensusWithProposalValue(p, prop, "payloadAccepted")
			consensusInfoString, _ := consensusInfo.JsonSerial()
			t.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoString)
			...
			consensusInfo := makeConsensusWithProposalValue(p, prop, "proposalCommittable")
			consensusInfoString, _ := consensusInfo.JsonSerial()
			t.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoString)
		}
	}
...
func (t *tracer) logVoteAggregatorResult(input filterableMessageEvent, output event) {
	...
	case softThreshold, certThreshold, nextThreshold:
		...
		consensusInfo := makeConsensusWithBundle(t.playerInfo, b, "bundleAccept")
		consensusInfoString, _ := consensusInfo.JsonSerial()
		t.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoString)
...
func (t *tracer) logVoteTrackerResult(p player, input voteAcceptedEvent, output thresholdEvent, weight uint64, inputTotal uint64, outputTotal uint64, proto config.ConsensusParams) {
	...
	consensusInfo := makeConsensusWithProposalVote(input.Vote, "voteAccept")
	consensusInfoStrin, _ := consensusInfo.JsonSerial()
	t.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoStrin)
...
func makeConsensusWithProposalAcceptedEvent(p player, pae proposalAcceptedEvent) consensus.ConsensusInfo {
	...
	consensusInfo.Propose = pae.Proposal.OriginalProposer.String()
	consensusInfo.MemoInfo = "proposalAccepted"
	return consensusInfo
}

func makeConsensusWithProposalValue(p player, pv proposalValue, memoInfo string) consensus.ConsensusInfo {
	var consensusInfo consensus.ConsensusInfo
	consensusInfo.Round = uint64(p.Round)
	consensusInfo.Period = uint64(p.Period)
	consensusInfo.Step = uint64(p.Step)
	consensusInfo.Propose = pv.OriginalProposer.String()
	consensusInfo.MemoInfo = memoInfo
	return consensusInfo
}

func makeConsensusWithBundle(p tracerMetadata, bd unauthenticatedBundle, memoInfo string) consensus.ConsensusInfo {
	var consensusInfo consensus.ConsensusInfo
	consensusInfo.Round = uint64(p.Round)
	consensusInfo.Period = uint64(p.Period)
	consensusInfo.Step = uint64(p.Step)
	consensusInfo.Propose = bd.Proposal.OriginalProposer.String()
	consensusInfo.MemoInfo = memoInfo
	return consensusInfo
}
