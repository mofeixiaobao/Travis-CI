...
import (
	...
	"github.com/gatechain/logging/logspec"
...
func (a networkAction) do(ctx context.Context, s *Service) {
	//fmt.Println("###, a.type:", a.T)
	if a.T == broadcastVotes {
		tag := protocol.AgreementVoteTag
		for i, uv := range a.UnauthenticatedVotes {
			data := protocol.Encode(uv)
			fmt.Println(" network round UnauthenticatedVotes: ", uv.R.Round, ",,,period:", uv.R.Period, ",,,step: ", uv.R.Step)
	...
	case protocol.AgreementVoteTag:
		...
		s.Ledger.UnauthenticatedCertificateSave(UnauthenticatedCertificate(a.UnauthenticatedVote))
		//fmt.Println(" network round AgreementVoteTagInfo: ", a.UnauthenticatedVote.R.Round,
		//	",,,period:", a.UnauthenticatedVote.R.Period, ",,,step: ", a.UnauthenticatedVote.R.Step,
		//	",,,sender:", a.UnauthenticatedVote.R.Sender, ",,,proposal:", a.UnauthenticatedVote.R.Proposal.OriginalProposer)
	case protocol.VoteBundleTag:
		data = protocol.Encode(a.UnauthenticatedBundle)
		//fmt.Println(" network round VoteBundleTagInfo: ",
		//	a.UnauthenticatedBundle.Round, ",,,period:", a.UnauthenticatedBundle.Period, ",,,step: ", a.UnauthenticatedBundle.Step,
		//	",,,VoteLength:", len(a.UnauthenticatedBundle.Votes), ",,,proposal:", a.UnauthenticatedBundle.Proposal.OriginalProposer)
	...
	case protocol.ProposalPayloadTag:
		...
		//fmt.Println(" network round ProposalPayloadTagInfo: ",
		//	msg.Vote.R.Round, ",,,period:", msg.Vote.R.Period, ",,,step: ", msg.Vote.R.Step,
		//	",,,sender:", msg.Vote.R.Sender, ",,,proposal:", msg.Vote.R.Proposal.OriginalProposer,
		//	",,,proposal2:", msg.Proposal.OriginalProposer)
...
func (a cryptoAction) do(ctx context.Context, s *Service) {
	switch a.T {
	case verifyVote:
		// logging.Base().Info(" cryptoAction verifyvote round: ", a.Round, ",,,period:", a.Period, ",,,step: ", ":default")
		s.demux.verifyVote(ctx, a.M, a.TaskIndex, a.Round, a.Period)
	case verifyPayload, verifyPayloadSelf:
		// logging.Base().Info(" cryptoAction verifyBundle round: ", a.Round, ",,,period:", a.Period, ",,,step: ", ":default")
		s.demux.verifyPayload(ctx, a.M, a.Round, a.Period, a.Pinned, a.T == verifyPayloadSelf)
	case verifyBundle:
		//fmt.Println(" cryptoAction verifyBundle round: ", a.Round, ",,,period:", a.Period, ",,,step: ", ":default")
...
func (a ensureAction) do(ctx context.Context, s *Service) {
	...
	if a.Payload.ve != nil {
		...
		if len(a.Certificate.Votes) > 1 {
			fmt.Println(time.Now().UTC(), " ensure round: ", a.Certificate.Round, ",,,period:", a.Certificate.Period, ",,,step: ", a.Certificate.Step,
				",,, proposal: ", a.Certificate.Proposal.OriginalProposer, ",,, proposalVoteLen:", len(a.Certificate.Votes),
				",,, proposal NO1:", a.Certificate.Votes[0].Sender, ",,, proposal NO2:", a.Certificate.Votes[1].Sender)
		} else {
			fmt.Println(time.Now().UTC(), " ensure round: ", a.Certificate.Round, ",,,period:", a.Certificate.Period, ",,,step: ", a.Certificate.Step,
				",,, proposal: ", a.Certificate.Proposal.OriginalProposer, ",,, proposalVoteLen:", len(a.Certificate.Votes),
				",,, proposal NO1:", a.Certificate.Votes[0].Sender)
		}
		...
		if !a.PayloadOk {
			...
			fmt.Println("2222 round: ", a.Certificate.Round, ",,,period:", a.Certificate.Period, ",,,step: ", a.Certificate.Step)
		} else {
			logEvent.Type = logspec.RoundConcluded
			...
			fmt.Println("3333 round: ", a.Certificate.Round, ",,,period:", a.Certificate.Period, ",,,step: ", a.Certificate.Step)
		}
	}
...
func (a rezeroAction) do(ctx context.Context, s *Service) {
	...
	s.ZeroTime = time.Now()
	logging.Base().Info("zero action enter, time is : %v", time.Now())
	fmt.Println(time.Now(), "zero action enter")
}
...
func (a pseudonodeAction) do(ctx context.Context, s *Service) {
	...
	case assemble:
		...
		//fmt.Println(" pseudonode assemble round: ", a.Round, ",,,period:", a.Period, ",,,step: ", a.Step)
		switch err {
		case nil:
	...
	case repropose:
		//fmt.Println(" pseudonode repropose round: ", a.Round, ",,,period:", a.Period, ",,,step: ", a.Step)
		...
		consensusInfo := makeConsensusWithPseudonodeAction(a, "repropose")
		consensusInfoString, _ := consensusInfo.JsonSerial()
		s.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoString)
	...
	case attest:
		//fmt.Println(" pseudonode attest round: ", a.Round, ",,,period:", a.Period, ",,,step: ", a.Step)
		...
		consensusInfo := makeConsensusWithPseudonodeAction(a, "VoteAttest")
		consensusInfoString, _ := consensusInfo.JsonSerial()
		s.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoString)
...
func verifyPayloadSelfAction(e messageEvent, r round, p period, pinned bool) action {
	return cryptoAction{T: verifyPayloadSelf, M: e.Input, Round: r, Period: p, Pinned: pinned}
}
...
func zeroAction(t actionType) action {
	...
	case verifyVote, verifyPayload, verifyPayloadSelf, verifyBundle:
		return cryptoAction{}
	case ensure:
		return ensureAction{}
	case rezero:
...
func (c checkpointAction) do(ctx context.Context, s *Service) {
	...
	if c.Err == nil {
		...
		//fmt.Println(" checkpoint round: ", c.Round, ",,,period:", c.Period, ",,,step: ", c.Step)
	...
	if c.done != nil {
		...
		fmt.Println(" checkpoint error: ", c.Round, ",,,period:", c.Period, ",,,step: ", c.Step)
...
func makeConsensusWithPseudonodeAction(pa pseudonodeAction, memoInfo string) consensus.ConsensusInfo {
	var consensusInfo consensus.ConsensusInfo
	consensusInfo.Round = uint64(pa.Round)
	consensusInfo.Period = uint64(pa.Period)
	consensusInfo.Step = uint64(pa.Step)
	consensusInfo.Propose = pa.Proposal.OriginalProposer.String()
