...
func MakeService(p Parameters, rootPath string) *Service {
	s := new(Service)
	...
	committeeLog := logging.NewLogger()
	logWriter := logging.MakeCyclicFileWriter(filepath.Join(rootPath, "committeeLog.log"), filepath.Join(rootPath, "committeeLog.archive.log"), p.LogSizeLimit, 0)
	committeeLog.SetOutput(logWriter)
	committeeLog.SetLevel(logging.Level(p.BaseLoggerDebugLevel))
	...
	s.tracer = makeTracer(s.log, committeeLog, defaultCadaverName, p.CadaverSizeTarget,
	...
	s.emptyChan = make(chan struct{}, 1)
...
func (s *Service) demuxLoop(ctx context.Context, input chan<- externalEvent, output <-chan []action, ready <-chan externalDemuxSignals) {
	for a := range output {
		s.do(ctx, a)
		extSignals := <-ready
		e, ok := s.demux.next(s, extSignals.Deadline, extSignals.FastRecoveryDeadline, extSignals.CurrentRound, extSignals.IsProPosal, extSignals.IsNeeToMonitorTx)
		if !ok {
			close(input)
			break
		}
...
func (s *Service) mainLoop(input <-chan externalEvent, output chan<- []action, ready chan<- externalDemuxSignals) {
	...
	for {
		...
		ready <- externalDemuxSignals{Deadline: status.Deadline, FastRecoveryDeadline: status.FastRecoveryDeadline, CurrentRound: status.Round,
			IsProPosal: status.IsMakeProposal, IsNeeToMonitorTx: status.IsMakeProposal}
		e, ok := <-input
		if !ok {
			break
		}
...
func (s *Service) GetPlayer() player {
	return s.persistStatus
}
