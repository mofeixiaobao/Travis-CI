...
type (
	...
	cryptoRequest struct {
		...
		Pinned    bool            // A flag that is set if this is a pinned value for the given round.
		ctx       context.Context // A context for this request, if the context is cancelled then the request is stale.
		self      bool            // A flag that is set if the proposal is produced by pseudo node
	}
...
func (c *poolCryptoVerifier) verifyProposalPayload(request cryptoRequest) cryptoResult {
	...
	p, err := up.validate(request.ctx, request.Round, request.Period, c.ledger, c.validator)
	select {
	case <-request.ctx.Done():
		m.Proposal = p
		return cryptoResult{message: m, TaskIndex: request.TaskIndex, Err: makeSerErr(request.ctx.Err()), Cancelled: true, self: request.self}
	default:
	}

	if err != nil {
		err := makeSerErrf("rejected invalid proposalPayload: %v", err)
		return cryptoResult{message: m, Err: err, TaskIndex: request.TaskIndex, self: request.self}
	}

	m.Proposal = p
	m.TxNum = len(p.PayProxySet)
	return cryptoResult{message: m, TaskIndex: request.TaskIndex, self: request.self}
}
