...
var Votes = make(map[string]int, 0)
var Payloads = make(map[string]int, 0)
		...
		for {
			...
			case raw, ok := <-networkMessages:
				...
				case protocol.AgreementVoteTag:
					r := o.(unauthenticatedVote).R
					if r.Step == 0 {
						p := r.Proposal.OriginalProposer.String()
						n, ok := Votes[p]
						if ok {
							Votes[p] = n + 1
						} else {
				...
				case protocol.ProposalPayloadTag:
					p := o.(compoundMessage).Proposal.OriginalProposer.String()
					n, ok := Payloads[p]
					if ok {
						Payloads[p] = n + 1
					} else {
...
func (d *demux) verifyPayload(ctx context.Context, m message, r round, p period, pinned bool, self bool) {
	d.UpdateEventsQueue(eventQueueCryptoVerifierProposal, 1)
	d.monitor.inc(cryptoVerifierCoserviceType)
	d.crypto.Verify(ctx, cryptoRequest{message: m, Round: r, Period: p, Pinned: pinned, self: self})
}
...
func (d *demux) next(s *Service, deadline time.Duration, fastDeadline time.Duration, currentRound round, isHaveProposal bool, isMonitorTx bool) (e externalEvent, ok bool) {
	defer func() {
		if !ok {
			return
		}
	...
	var syncComplete <-chan bool
	var deadlineCh <-chan time.Time
	if s.Syncing {
		s.Syncing, _ = s.Catchup.IsSynchronizing()
	}
	if s.Syncing {
		syncComplete = s.Catchup.SyncComplete()
	} else {
	...
	var proposalDeadlineCh <-chan time.Time
	if !isHaveProposal {
		if err == nil && waitForTxs(config.Consensus[proto]) {
			if (deadline-filterTimeout) > 0 && deadline == getCreateEmptyBlocksInterval(config.Consensus[proto]) {
				proposalDeadlineCh = s.Clock.TimeoutAt(deadline - filterTimeout)
			}
		}
	}
	...
	var monitorTxChan <-chan struct{}
	if isMonitorTx {
		monitorTxChan = s.emptyChan
	} else {
		monitorTxChan = s.BlockValidator.TxsAvailabled()
	}
	...
	case e, ok = <-pseudonodeEvents:
		...
		d.queue = d.queue[1:]
		d.UpdateEventsQueue(eventQueuePseudonode, 0)
		return d.next(s, deadline, fastDeadline, currentRound, isHaveProposal, isMonitorTx)
	...
	case r := <-d.crypto.Verified(protocol.ProposalPayloadTag):
		var et eventType
		if r.self {
			et = payloadVerifiedSelf
		} else {
			et = payloadVerified
		}
		e = messageEvent{T: et, Input: r.message, Err: r.Err, Cancelled: r.Cancelled}
		d.UpdateEventsQueue(eventQueueDemux, 1)
	...
	case r := <-d.crypto.Verified(protocol.VoteBundleTag):
		...
		d.monitor.inc(demuxCoserviceType)
		d.monitor.dec(cryptoVerifierCoserviceType)
	case <-proposalDeadlineCh:
		e = timeoutEvent{T: proposalTimeout, RandomEntropy: s.RandomSource.Uint64(), Round: nextRound}
		d.UpdateEventsQueue(eventQueueDemux, 1)
		d.monitor.inc(demuxCoserviceType)
		d.monitor.dec(clockCoserviceType)
	case <-monitorTxChan:
		logging.Base().Info("demux have new tx arrived")
		fmt.Println(time.Now().UTC(), "demux have new tx arrived")
		e = timeoutEvent{T: newTxArrived, RandomEntropy: s.RandomSource.Uint64(), Round: nextRound}
		d.UpdateEventsQueue(eventQueueDemux, 1)
		d.monitor.inc(demuxCoserviceType)
		d.monitor.dec(clockCoserviceType)
	}
...
func setupCompoundMessage(l LedgerReader, m message) (res externalEvent) {
	...
	if compound.Vote == (unauthenticatedVote{}) {
		//if compareUnauthenticatedVote(compound.Vote) {
...
func waitForTxs(cfg config.ConsensusParams) bool {
	return !cfg.CreateEmptyBlocks || cfg.CreateEmptyBlocksInterval > 0
}

func getCreateEmptyBlocksInterval(cfg config.ConsensusParams) time.Duration {
