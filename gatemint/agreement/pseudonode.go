...
func (n asyncPseudonode) makeProposals(round basics.Round, period period, accounts []account.Participation) ([]proposal, []unauthenticatedVote) {
	...
	ve, err := n.factory.AssembleBlock(round, uint64(period), deadline, CommitteeBottomPeriod)
	if err != nil {
...
func (n asyncPseudonode) makeVotes(round basics.Round, period period, step step, proposal proposalValue, participation []account.Participation) []unauthenticatedVote {
	...
	for _, account := range participation {
		...
		consensusInfo := makeConsensusWithRawVote(rv, "makeVote")
		consensusInfoString, _ := consensusInfo.JsonSerial()
		n.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoString)
...
func (t pseudonodeVotesTask) execute(verifier *AsyncVoteVerifier, quit chan struct{}) {
	...
	for _, result := range verifiedResults {
		...
		consensusInfo := makeConsensusWithProposalVote(vote, "voteBroadcast")
		consensusInfoStrin, _ := consensusInfo.JsonSerial()
		t.node.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoStrin)
...
func (t pseudonodeProposalsTask) execute(verifier *AsyncVoteVerifier, quit chan struct{}) {
	...
	for i := range cryptoOutputs {
		...
		consensusInfo := makeConsensusWithProposalVote(vote, "makeProposal")
		consensusInfoStrin, _ := consensusInfo.JsonSerial()
		t.node.log.Infof("%s:%s", consensus.ConsensusTag, consensusInfoStrin)
	}
...
func makeConsensusWithProposalVote(vote vote, memoInfo string) consensus.ConsensusInfo {
	var consensusInfo consensus.ConsensusInfo
	consensusInfo.Round = uint64(vote.R.Round)
	consensusInfo.Period = uint64(vote.R.Period)
	consensusInfo.Step = uint64(vote.R.Step)
	consensusInfo.ProposeList = append(consensusInfo.ProposeList, vote.R.Proposal.OriginalProposer.String())
	consensusInfo.Sender = vote.R.Sender.String()
	consensusInfo.MemoInfo = memoInfo
	return consensusInfo
}

func makeConsensusWithRawVote(vote rawVote, memoInfo string) consensus.ConsensusInfo {
	var consensusInfo consensus.ConsensusInfo
	consensusInfo.Round = uint64(vote.Round)
	consensusInfo.Period = uint64(vote.Period)
	consensusInfo.Step = uint64(vote.Step)
	consensusInfo.ProposeList = append(consensusInfo.ProposeList, vote.Proposal.OriginalProposer.String())
	consensusInfo.Sender = vote.Sender.String()
	consensusInfo.MemoInfo = memoInfo
	return consensusInfo
}
