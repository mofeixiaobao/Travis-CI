...
type (
	...
	unauthenticatedEquivocationVote struct {
		...
		Sigs      [2]basics.OneTimeSignature          `codec:"sigs"`
	}
...
func makeVote(rv rawVote, voting basics.OneTimeSigner, selection *crypto.VRFSecrets, l Ledger) (unauthenticatedVote, error) {
	m, err := membership(l, rv.Sender, rv.Round, rv.Period, rv.Step)
	if err != nil {
	...
	if (sig == basics.OneTimeSignature{}) {
...
func (rv rawVote) ToBeHashed() (crypto.HashID, []byte) {
