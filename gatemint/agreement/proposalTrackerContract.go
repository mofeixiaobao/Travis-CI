...
func (c *proposalTrackerContract) pre(p player, in event) (pre []error) {
	switch in.t() {
	case voteVerified, proposalFrozen, committeeFrozen, softThreshold, voteFilterRequest, readStaging:
	default:
...
func (c *proposalTrackerContract) post(p player, in, out event) (post []error) {
	...
	case committeeFrozen:
		if out.t() != committeeFrozen {
			post = append(post, fmt.Errorf("output event from committeeFrozen has bad type: %v", out.t()))
		}
		_, ok := out.(committeeFrozenEvent)
		if !ok {
			post = append(post, fmt.Errorf("output event does not cast to committeeFrozenEvent: output is %#v", out))
		}
