...
type PendingTxAggregate interface {
	...
	ProxyPendingTxIDs() []transactions.Txid
	ProxyPending() []transactions.TxWithValidInfo
}
...
type ProxyTxSyncClient interface {
	ProxySync(ctx context.Context, bloom *bloom.Filter) (txns []transactions.Tx, err error)
	Address() string
	Close() error
}
...
type TxSyncer struct {
	...
	log             logging.Logger
	httpSync        *HTTPTxSync
	httpProxyTxSync *HTTPProxyTxSync
}
...
func MakeTxSyncer(pool PendingTxAggregate, clientSource PeerSource, txHandler data.SolicitedTxHandler, syncInterval time.Duration, syncTimeout time.Duration, serverResponseSize int) *TxSyncer {
	...
	return &TxSyncer{
		...
		httpProxyTxSync: makeHTTPProxyTxSync(clientSource, logging.Base(), uint64(serverResponseSize)),
	}
}
	...
	go func() {
		...
		for {
			...
			case <-time.After(syncer.syncInterval):
				// replace from std tx sync to proxy tx sync
				err := syncer.syncProxy()
				if err != nil {
...
func (syncer *TxSyncer) syncProxy() error {
	return syncer.syncProxyFromClient(syncer.httpProxyTxSync)
}
...
func (syncer *TxSyncer) syncProxyFromClient(client ProxyTxSyncClient) error {
	syncer.log.Infof("TxSyncer.Sync: asking client %v for missing transactions", client.Address())

	pending := syncer.pool.ProxyPendingTxIDs()
	sizeBits, numHashes := bloom.Optimal(len(pending), bloomFilterFalsePositiveRate)
	filter := bloom.New(sizeBits, numHashes, syncer.counter)
	syncer.counter++
	for _, txid := range pending {
	...
	ctx, cf := context.WithTimeout(syncer.ctx, syncer.syncTimeout)
	defer cf()
	txgroups, err := client.ProxySync(ctx, filter)
	if err != nil {
		return fmt.Errorf("TxSyncer.Sync: peer %v error %v", client.Address(), err)
	}
	// test to see if all the transaction that we've received honor the bloom filter constraints
	...
	for _, txgroup := range txgroups {
		...
		// if the entire group was in the bloom filter, report an error.
		//if txnsInFilter == len(txgroup.TxInfo) {
		//	client.Close()
		//	return fmt.Errorf("TxSyncer.Sync: peer %v sent a transaction group that was entirely included in the bloom filter", client.Address())
		//}
	}
	// send the transaction to the trasaction pool
	if syncer.handler.HandleProxy(txgroups) != nil {
		client.Close()
		return fmt.Errorf("ProxyTxSyncer.ProxySync: peer %v sent invalid transaction", client.Address())
	}
