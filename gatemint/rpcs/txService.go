...
import (
	...
	"github.com/gatechain/gatemint/util/bloom"
...
type TxService struct {
	...
	pengdingProxyTxGroups []transactions.Tx
	lastUpdate            int64
	mu                    deadlock.RWMutex
	genesisID             string
	log                   logging.Logger
...
func (txs *TxService) ServeHTTP(response http.ResponseWriter, request *http.Request) {
	...
	// here replace std txn to proxy tx
	//txns := txs.getFilteredTxns(filter)
	//txblob := protocol.Encode(txns)

	txns := txs.getFilteredProxyTxns(filter)
	txblob := protocol.Encode(txns)
...
func (txs *TxService) getFilteredProxyTxns(bloom *bloom.Filter) (txns []transactions.Tx) {
	pendingProxyTxGroups := txs.updateProxyTxCache()

	missingTxns := make([]transactions.Tx, 0)
	encodedLength := 0
	for _, txgroup := range pendingProxyTxGroups {
		missing := false
		txGroupLength := 0
		id := txgroup.ComputeID()
		if !bloom.Test(id[:]) {
			missing = true
		}
		txGroupLength += txgroup.ComputeEncodingLen()
...
func (txs *TxService) updateProxyTxCache() (pendingTxGroups []transactions.Tx) {
	currentUnixTime := time.Now().Unix()
	txs.mu.RLock()
	if txs.lastUpdate != 0 && txs.lastUpdate+updateInterval >= currentUnixTime {
		// no need to update.
		pendingTxGroups = txs.pengdingProxyTxGroups
		txs.mu.RUnlock()
		return
	}
	txs.mu.RUnlock()
	...
	if txs.lastUpdate == 0 || txs.lastUpdate+updateInterval < currentUnixTime {
		...
		// array ( if being used ) is still valid. There is no risk of data race here since
		// the txs.pendingTxGroups is a slice (hence a pointer to the array) and not the array itself.
		proxyTxPending := txs.pool.ProxyPending()
		txGroups := make([]transactions.Tx, 0)
		for _, proxyTxInfo := range proxyTxPending {
			txGroups = append(txGroups, proxyTxInfo.TxInfo)
		}
		txs.pengdingProxyTxGroups = txGroups
		txs.lastUpdate = currentUnixTime
	}
	return txs.pengdingProxyTxGroups
}

// TxServiceHTTPPath is the URL path to sync pending transactions from
const TxServiceHTTPPath = "/v1/{genesisID}/txsync"

// ProxyTxServiceHTTPPath is the URL path to sync pending proxy transactions from
//const ProxyTxServiceHTTPPath = "/v1/{genesisID}/proxytxsync"
