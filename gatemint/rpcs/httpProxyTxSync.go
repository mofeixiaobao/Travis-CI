...
func makeHTTPProxyTxSync(peerSource PeerSource, log logging.Logger, serverResponseSize uint64) *HTTPProxyTxSync {
...
func (hts *HTTPProxyTxSync) ProxySync(ctx context.Context, bloom *bloom.Filter) (txgroups []transactions.Tx, err error) {
	...
	default:
		hts.log.Warn("proxy txSync response status code : ", response.StatusCode)
		response.Body.Close()
		return nil, fmt.Errorf("proxy txSync POST error response status code %d", response.StatusCode)
	}
	...
	if len(contentTypes) != 1 {
		err = fmt.Errorf("proxy txSync POST invalid content type count %d", len(contentTypes))
		hts.log.Warn(err)
		response.Body.Close()
		return nil, err
	}
	...
	if err != nil {
		hts.log.Warn("proxy txSync body read failed: ", err)
		return nil, err
	}
	hts.log.Debugf("http sync got %d bytes", len(data))

	var txns []transactions.Tx
	err = protocol.Decode(data, &txns)
	if err != nil {
		hts.log.Warn("proxy txSync protocol decode: ", err)
	}
...
func (hts *HTTPProxyTxSync) Address() string {
	return hts.rootURL
}
...
func (hts *HTTPProxyTxSync) Close() error {
	return nil
}
