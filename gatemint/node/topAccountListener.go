...
type topAccountListener struct {
	...
	onlineCirculation basics.Power

	totalCirculation basics.Power
...
func (t *topAccountListener) update(b bookkeeping.Block, balances basics.BalanceDetail) {
	...
	smallestAccountSize := basics.Power{Raw: 0}
	if len(t.accounts) != 0 {
		smallestAccountSize = t.accounts[len(t.accounts)-1].Power
	}
	...
	for i, acct := range t.accounts {
		if acct.Power.LessThan(smallestAccountSize) {
...
func updateTopAccounts(topN []basics.AccountDetail, balances []basics.AccountDetail) []basics.AccountDetail {
	for _, account := range balances {
		balance := account.Power

		// Quick check for topN if capacity is reached.
		if account.Status != basics.Online || len(topN) != 0 && len(topN) == cap(topN) && balance.Raw <= topN[len(topN)-1].Power.Raw {
			continue
		}

		// Find insertion point.
		pos := sort.Search(len(topN), func(i int) bool {
			return topN[i].Power.LessThan(balance)
		})
...
func (t *topAccountListener) sendEvent() {
	...
	for _, account := range t.accounts[:] {
		...
		entry["balance"] = account.Power.ToUint64()
		entry["stake"] = float64(account.Power.ToUint64()) / fCirculation
