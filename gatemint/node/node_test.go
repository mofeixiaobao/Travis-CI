...
func setupFullNodes(t *testing.T, proto protocol.ConsensusVersion, verificationPool execpool.BacklogPool) ([]*GatemintFullNode, []string, []string) {
	...
	nodes := make([]*GatemintFullNode, numAccounts)
...
func startAndConnectNodes(nodes []*GatemintFullNode, delayStartFirstNode bool) {
...
func connectPeers(nodes []*GatemintFullNode) {
...
func delayStartNode(node *GatemintFullNode, peers []*GatemintFullNode, delay time.Duration) {
	var wg sync.WaitGroup
	wg.Add(1)
	go func() {
		defer wg.Done()
