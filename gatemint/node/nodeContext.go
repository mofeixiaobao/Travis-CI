...
func (node *GatemintFullNode) IsCatchingUp() bool {
...
func (node *GatemintFullNode) IsInitialCatchupComplete() bool {
	// Lock not required - syncer doesn't change
...
func (node *GatemintFullNode) HasCaughtUp() bool {
	node.muData.RLock()
...
func (node *GatemintFullNode) SetLastLiveRound(round basics.Round) {
	node.muData.Lock()
