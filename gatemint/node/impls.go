...
func (i blockValidatorImpl) Validate(ctx context.Context, period uint64, e bookkeeping.Block) (agreement.ValidatedBlock, error) {
	b := &e
	lvb, err := i.l.Validate(ctx, *b, i.tp, i.verificationPool, period)
	if err != nil {
		return nil, err
	}
...
func (i blockValidatorImpl) TxsAvailabled() <-chan struct{} {
	return i.tp.TxsAvailabled()
}
...
func (i *blockFactoryImpl) AssembleBlock(round basics.Round, period uint64, deadline time.Time, committeeBottomPeriod uint64) (agreement.ValidatedBlock, error) {
	start := time.Now()
	...
	if round > 2 {
		lastBlockCertificate := i.l.ReadPreCertificate()
		if lastBlockCertificate.Round != (round - 1) {
			logging.Base().Warnf("ledger preCertificate round is not right, round is %v, need round is %v ", lastBlockCertificate.Round, round-1)
	...
	newEmptyBlk := bookkeeping.MakeBlock(prev, committee, equivocations, proposer, appState)
	...
	// todo need to change to assemblePayProxySet
...
func (i *blockFactoryImpl) getBalancePower(round basics.Round, address basics.Address) uint64 {
...
func (l agreementLedger) FetchBlockByDigest(round basics.Round, quit chan struct{}) (bookkeeping.Block, agreement.Certificate, error) {
	...
	for {
		...
		case <-retryTicker.C:
			logging.Base().Infof("error fetching block (%v), trying again", err)
		}
	}
}
