...
import (
	...
	kmdclient "github.com/gatechain/gatemint/daemon/kmd/client"
...
// DefaultKMDDataDir is the name of the directory within the gmd data directory where kmd data goes
...
type ClientConfig struct {
	// AlgodDataDir is the data dir for `gmd`
	AlgodDataDir string
...
const (
	...
	KmdClient
	// AlgodClient ensures the gmd client can be initialized when created
	AlgodClient
...
func (c *Client) ConstructPayment(from, to string, fee, amount uint64, note []byte, closeTo string, lease [32]byte, firstValid, lastValid basics.Round) (transactions.Transaction, error) {
	...
	tx := transactions.Transaction{
		...
		PaymentTxnFields: transactions.PaymentTxnFields{
			Receiver: toAddr,
			Amount:   basics.Power{Raw: amount},
		},
	}
