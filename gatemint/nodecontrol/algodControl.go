...
func (nc *NodeController) StartAlgod(args AlgodStartArgs) (alreadyRunning bool, err error) {
	...
	gmdCmd := nc.buildAlgodCommand(args)
	...
	if args.RedirectOutput {
		...
		linePrefix := fmt.Sprintf("gmd(%d)", gmdCmd.Process.Pid)
