...
import (
	...
	"github.com/gatechain/gatemint/util/codecs"
)
...
type ConsensusParams struct {
	...
	// CommitteeNum specifies the committee num in block
	CommitteeNum int

	// CommitteeBottomPeriod specifies the committee bottom period
	CommitteeBottomPeriod uint64
...
func initConsensusProtocols() {
	...
	// Base consensus protocol version, v1.
	v1 := ConsensusParams{
		...
		MaxTxnBytesPerBlock: 500000,
		DefaultKeyDilution:  100,

		CreateEmptyBlocks:         false,
		CreateEmptyBlocksInterval: 60 * time.Second,
		...
		CommitteeBottomPeriod: 3,

		MaxTimestampIncrement: 100,
	...
	v1.ApprovedUpgrades = map[protocol.ConsensusVersion]bool{}
	Consensus[protocol.ConsensusV1] = v1
...
type Local struct {
	...
	// To make sure the gmd process does not run out of FDs, algod ensures
	...
	// TxSortedType decides tx sorted type in transactionPool
	// 0: sorted by tx accepting order
	// 1: sorted by tx fee descending order
	TxSortedType int
	...
	// First participation consensus round
	FirstPartKeyRound uint64

	// Last participation consensus round
	LastPartKeyRound uint64

	RefreshPartInterval uint64

	// Minimum packing charge for blocks
	// Warning : This configuration conflicts with the CreateEmptyBlocks configuration.
	// This configuration weight is higher than CreateEmptyBlocks configuration.
	MinimumTxFee uint64
}
...
func loadConfigFromFile(configFile string) (c Local, err error) {
	c = defaultLocal
	c.Version = 1 // Reset to 0 so we get the version from the loaded file.
...
func (cfg Local) SaveAs(root string) error {
	configpath := filepath.Join(root, ConfigFilename)
	filename := os.ExpandEnv(configpath)

	// Turn on property switch
	cfg.IsIndexerActive = false
	cfg.Archival = true
	// Default DNS url
	cfg.DNSBootstrapID = "<network>.gatenode.cc"

	return codecs.SaveObjectToFile(filename, cfg, true)
}

// SetRoot sets the RootDir for all Config structs
func (cfg *Local) SetRoot(root string) *Local {
	cfg.RootDir = root
	return cfg
}

// GenesisFile returns the full path to the genesis.json file
func (cfg Local) GenesisFile() string {
	return filepath.Join(cfg.RootDir, GenesisJSONFile)
}
