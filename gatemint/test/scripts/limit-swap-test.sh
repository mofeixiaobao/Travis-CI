...
goal network create -r ${NETDIR} -n tbd -t ${GOPATH}/src/github.com/gatechain/gatemint/test/testdata/nettemplates/TwoNodes50EachFuture.json
...
sed s/TMPL_ASSET/${ASSET_ID}/g < ${GOPATH}/src/github.com/gatechain/gatemint/tools/teal/templates/limit-order-a.teal.tmpl | sed s/TMPL_SWAPN/31337/g | sed s/TMPL_SWAPD/137/g | sed s/TMPL_TIMEOUT/${TIMEOUT_ROUND}/g | sed s/TMPL_OWN/${ACCOUNT}/g | sed s/TMPL_FEE/100000/g | sed s/TMPL_MINTRD/10000/g > ${TEMPDIR}/limit-order-a.teal
...
sed s/TMPL_ASSET/${ASSET_ID}/g < ${GOPATH}/src/github.com/gatechain/gatemint/tools/teal/templates/limit-order-b.teal.tmpl | sed s/TMPL_SWAPN/137/g | sed s/TMPL_SWAPD/31337/g | sed s/TMPL_TIMEOUT/${TIMEOUT_ROUND}/g | sed s/TMPL_OWN/${ACCOUNT}/g | sed s/TMPL_FEE/100000/g | sed s/TMPL_MINTRD/10000/g > ${TEMPDIR}/limit-order-b.teal
