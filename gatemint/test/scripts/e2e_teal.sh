...
${BINDIR}/goal network create -r ${NETDIR} -n tbd -t ${GOPATH}/src/github.com/gatechain/gatemint/test/testdata/nettemplates/TwoNodes50EachFuture.json
...
python ${GOPATH}/src/github.com/gatechain/gatemint/data/transactions/logic/tlhc.py --from ${ACCOUNT} --to ${ACCOUNTB} --timeout-round ${TIMEOUT_ROUND} > ${TEMPDIR}/tlhc.teal 2> ${TEMPDIR}/tlhc.teal.secret
